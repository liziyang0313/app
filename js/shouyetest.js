...
function setCook() {
    ...
    if(id){
        var cookBox=[];
        if(localStorage.getItem('pagecook')){
        	cookBox=localStorage.getItem('pagecook').split(',');
        }
        if(cookBox.indexOf(id)<=-1){
        	cookBox.push(id);
        }
        localStorage.setItem('pagecook',cookBox.toString());
    }
}
...
function  getHistory() {
    var cookBox=[];
    if(localStorage.getItem('pagecook')){
    	cookBox=localStorage.getItem('pagecook');
    };
    var historydata=allData.filter(function (item) {
        return cookBox.indexOf(item.id)>-1;
    });
